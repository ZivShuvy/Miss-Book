...
let gBooks = []
let gSearch = storageService.loadFromStorage('searchesDB') || {};
const KEY = 'bookDB';
...
function query(filterBy) {
    if (filterBy) {
        ...
        const booksToShow = gBooks.filter(book => book.title.startsWith(title.toLowerCase()) && book.listPrice.amount >= minPrice && book.listPrice.amount <= maxPrice);
...
function getResults(txt) {
    return new Promise((resolve, reject) => {
        if (gSearch[txt.toLowerCase()]) {
            resolve(gSearch[txt]);
        }
        axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${txt}`)
            .then(res => {
                const results = res.data.items.slice(0, 5);
                gSearch[txt.toLowerCase()] = results;
...
function addGoogleBook(googleBook) {
    const isExisted = gBooks.some(book => book.id === googleBook.id);
    if (isExisted) {
        return Promise.reject();
    }
    const book = {
        id: googleBook.id,
        title: googleBook.volumeInfo.title.toLowerCase(),
        ...
        listPrice: {
            ...
            currencyCode: utilService.getRandCurrencyCode(),
...
function getNextBookId(bookId, diff) {
    ...
    else if (nextbookIdx < 0) nextbookIdx = gBooks.length - 1
...
function _saveSearchToStorage() {
    storageService.saveToStorage('searchesDB', gSearch);
}
...
function _createBooks() {
    if (storageService.loadFromStorage(KEY)) {
